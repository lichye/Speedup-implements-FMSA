...
//  errs() << "Verifier: " << I.getOpcodeName() << ": " << Broken << "\n";
...
//  II.dump();
//  errs() << "Verifier: visit invoke 1: " << Broken << "\n";
  verifyCallSite(&II);
//  errs() << "Verifier: visit invoke 2: " << Broken << "\n";

//  if (!II.getUnwindDest()->isEHPad()) II.getUnwindDest()->dump();
...
//  errs() << "Verifier: visit invoke 3: " << Broken << "\n";
  visitTerminatorInst(II);
//  errs() << "Verifier: visit invoke 4: " << Broken << "\n";
}
...
//  errs() << "Verifier: visit GEP 1: " << Broken << "\n";

  Assert(isa<PointerType>(TargetTy),
         "GEP base pointer is not a vector or a vector of pointers", &GEP);

//  errs() << "Verifier: visit GEP 2: " << Broken << "\n";

  Assert(GEP.getSourceElementType()->isSized(), "GEP into unsized type!", &GEP);

//  errs() << "Verifier: visit GEP 3: " << Broken << "\n";
...
//  errs() << "Verifier: visit GEP 4: " << Broken << "\n";
//  for (Value *IdxV : Idxs) {
//    IdxV->dump();
//  }
...
//  errs() << "Verifier: visit GEP 5: " << Broken << "\n";

  Assert(GEP.getType()->isPtrOrPtrVectorTy() &&
             GEP.getResultElementType() == ElTy,
         "GEP is not of right type for indices!", &GEP, ElTy);

//  errs() << "Verifier: visit GEP 6: " << Broken << "\n";

  if (GEP.getType()->isVectorTy()) {
    // Additional checks for vector GEPs.
    unsigned GEPWidth = GEP.getType()->getVectorNumElements();
    if (GEP.getPointerOperandType()->isVectorTy()) {
//      errs() << "Verifier: visit GEP 7: " << Broken << "\n";
      Assert(GEPWidth == GEP.getPointerOperandType()->getVectorNumElements(),
             "Vector GEP result width doesn't match operand's", &GEP);
    }
//    errs() << "Verifier: visit GEP 8: " << Broken << "\n";
    for (Value *Idx : Idxs) {
...
//      errs() << "Verifier: visit GEP 9: " << Broken << "\n";

      Assert(IndexTy->isIntOrIntVectorTy(),
             "All GEP indices should be of integer type");
    }
//    errs() << "Verifier: visit GEP 10: " << Broken << "\n";
  }
//  errs() << "Verifier: visit GEP 11: " << Broken << "\n";
  visitInstruction(GEP);
//  errs() << "Verifier: visit GEP 12: " << Broken << "\n";
}
...
//  SI.dump();
//  errs() << "Verifier: Store Inst 1: " << Broken << "\n";

  PointerType *PTy = dyn_cast<PointerType>(SI.getOperand(1)->getType());
  Assert(PTy, "Store operand must be a pointer.", &SI);

//  errs() << "Verifier: Store Inst 2: " << Broken << "\n";

  Type *ElTy = PTy->getElementType();
  Assert(ElTy == SI.getOperand(0)->getType(),
         "Stored value type does not match pointer operand type!", &SI, ElTy);

//  errs() << "Verifier: Store Inst 3: " << Broken << "\n";

  Assert(SI.getAlignment() <= Value::MaximumAlignment,
         "huge alignment values are unsupported", &SI);

//  errs() << "Verifier: Store Inst 4: " << Broken << "\n";

  Assert(ElTy->isSized(), "storing unsized types is not allowed", &SI);

//  errs() << "Verifier: Store Inst 5: " << Broken << "\n";

  if (SI.isAtomic()) {
//    errs() << "Verifier: Store Inst 6: " << Broken << "\n";

    Assert(SI.getOrdering() != AtomicOrdering::Acquire &&
               SI.getOrdering() != AtomicOrdering::AcquireRelease,
           "Store cannot have Acquire ordering", &SI);

//    errs() << "Verifier: Store Inst 7: " << Broken << "\n";
    Assert(SI.getAlignment() != 0,
           "Atomic store must specify explicit alignment", &SI);

//    errs() << "Verifier: Store Inst 8: " << Broken << "\n";
...
//    errs() << "Verifier: Store Inst 9: " << Broken << "\n";
 
    checkAtomicMemAccessSize(ElTy, &SI);
  } else {
//    errs() << "Verifier: Store Inst 10: " << Broken << "\n";

    Assert(SI.getSyncScopeID() == SyncScope::System,
           "Non-atomic store cannot have SynchronizationScope specified", &SI);
  }

//  errs() << "Verifier: Store Inst 11: " << Broken << "\n";
  visitInstruction(SI);
//  errs() << "Verifier: Store Inst 12: " << Broken << "\n";
}
...
//  errs() << "Verifier: visit EHPad 1: " << Broken << "\n";
  assert(I.isEHPad());

//  errs() << "Verifier: visit EHPad 2: " << Broken << "\n";
...
//  errs() << "Verifier: visit EHPad 3: " << Broken << "\n";
...
/*
      errs() << "Verifier: visit EHPad 4: " << Broken << "\n";
      if (!II) {
        errs() << "Verifier: visit EHPad 4: Error 1\n";
        PredBB->dump();
      }
      if (II && II->getUnwindDest() != BB) {
        errs() << "Verifier: visit EHPad 4: Error 2\n";
      }
      if (II && II->getNormalDest() != BB) {
        errs() << "Verifier: visit EHPad 4: Error 3\n";
      }
*/
...
//       errs() << "Verifier: visit EHPad 5: " << Broken << "\n";
  
    }
    return;
  }
  if (auto *CPI = dyn_cast<CatchPadInst>(&I)) {
//    errs() << "Verifier: visit EHPad 6: " << Broken << "\n";
    if (!pred_empty(BB))
...
//      errs() << "Verifier: visit EHPad 7: " << Broken << "\n";

    Assert(BB != CPI->getCatchSwitch()->getUnwindDest(),
           "Catchswitch cannot unwind to one of its catchpads",
           CPI->getCatchSwitch(), CPI);

//    errs() << "Verifier: visit EHPad 8: " << Broken << "\n";

    return;
  }

//  errs() << "Verifier: visit EHPad 9: " << Broken << "\n";
...
//  errs() << "Verifier: visit EHPad 10: " << Broken << "\n";
}
...
//  LPI.dump();
//  errs() << "Verifier: before predecessor: " << Broken << "\n";

  visitEHPadPredecessors(LPI);
//  errs() << "Verifier: After predecessor: " << Broken << "\n";
...
//  errs() << "Verifier: visit LandingPad 1: " << Broken << "\n";

  Function *F = LPI.getParent()->getParent();
  Assert(F->hasPersonalityFn(),
         "LandingPadInst needs to be in a function with a personality.", &LPI);

//  errs() << "Verifier: visit LandingPad 2: " << Broken << "\n";

//  if (LPI.getParent()->getLandingPadInst() != &LPI) {
//    LPI.getParent()->dump();
//  }
...
//  errs() << "Verifier: visit LandingPad 3: " << Broken << "\n";

  for (unsigned i = 0, e = LPI.getNumClauses(); i < e; ++i) {
//    errs() << "Verifier: visit LandingPad 3, " << i << " : " << Broken << "\n";
...
//  errs() << "Verifier: visit LandingPad 4: " << Broken << "\n";
  visitInstruction(LPI);
//  errs() << "Verifier: visit LandingPad 5: " << Broken << "\n";
}
...
void Verifier::verifyDominatesUse(Instruction &I, unsigned i) {
  ...
  //I.dump();
  //errs() << "Verifier: dominator 1: " << Broken << "\n";
...
//  errs() << "Verifier: visit Instr 1: " << Broken << "\n";
...
//  errs() << "Verifier: visit Instr 2: " << Broken << "\n";

  // Check that void typed values don't have names
  Assert(!I.getType()->isVoidTy() || !I.hasName(),
         "Instruction has a name, but provides a void value!", &I);

//  errs() << "Verifier: visit Instr 3: " << Broken << "\n";
...
//  errs() << "Verifier: visit Instr 4: " << Broken << "\n";
...
//  errs() << "Verifier: visit Instr 5: " << Broken << "\n";
...
//  errs() << "Verifier: visit Instr 6: " << Broken << "\n";

  for (unsigned i = 0, e = I.getNumOperands(); i != e; ++i) {

//    errs() << "Verifier: visit Instr 7: " << Broken << "\n";

    Assert(I.getOperand(i) != nullptr, "Instruction has null operand!", &I);

//    errs() << "Verifier: visit Instr 8: " << Broken << "\n";
...
//    errs() << "Verifier: visit Instr 9: " << Broken << "\n";
...
//      errs() << "Verifier: visit Instr 10: " << Broken << "\n";
...
//      errs() << "Verifier: visit Instr 11: " << Broken << "\n";

      Assert(F->getParent() == &M, "Referencing function in another module!",
             &I, &M, F, F->getParent());

//      errs() << "Verifier: visit Instr 12: " << Broken << "\n";

    } else if (BasicBlock *OpBB = dyn_cast<BasicBlock>(I.getOperand(i))) {

//      errs() << "Verifier: visit Instr 13: " << Broken << "\n";

      Assert(OpBB->getParent() == BB->getParent(),
             "Referring to a basic block in another function!", &I);

//      errs() << "Verifier: visit Instr 14: " << Broken << "\n";

    } else if (Argument *OpArg = dyn_cast<Argument>(I.getOperand(i))) {

//      errs() << "Verifier: visit Instr 15: " << Broken << "\n";

      Assert(OpArg->getParent() == BB->getParent(),
             "Referring to an argument in another function!", &I);

//      errs() << "Verifier: visit Instr 16: " << Broken << "\n";

    } else if (GlobalValue *GV = dyn_cast<GlobalValue>(I.getOperand(i))) {

//      errs() << "Verifier: visit Instr 17: " << Broken << "\n";

      Assert(GV->getParent() == &M, "Referencing global in another module!", &I,
             &M, GV, GV->getParent());

//      errs() << "Verifier: visit Instr 18: " << Broken << "\n";

    } else if (isa<Instruction>(I.getOperand(i))) {

//      errs() << "Verifier: visit Instr 19: " << Broken << "\n";
      verifyDominatesUse(I, i);

//      errs() << "Verifier: visit Instr 20: " << Broken << "\n";

    } else if (isa<InlineAsm>(I.getOperand(i))) {

//      errs() << "Verifier: visit Instr 21: " << Broken << "\n";

      Assert((i + 1 == e && isa<CallInst>(I)) ||
                 (i + 3 == e && isa<InvokeInst>(I)),
             "Cannot take the address of an inline asm!", &I);

//      errs() << "Verifier: visit Instr 22: " << Broken << "\n";

    } else if (ConstantExpr *CE = dyn_cast<ConstantExpr>(I.getOperand(i))) {

//      errs() << "Verifier: visit Instr 23: " << Broken << "\n";
...
//      errs() << "Verifier: visit Instr 24: " << Broken << "\n";

    }
  }

//  errs() << "Verifier: visit Instr 25: " << Broken << "\n";

  if (MDNode *MD = I.getMetadata(LLVMContext::MD_fpmath)) {

//    errs() << "Verifier: visit Instr 27: " << Broken << "\n";

    Assert(I.getType()->isFPOrFPVectorTy(),
           "fpmath requires a floating point result!", &I);

//    errs() << "Verifier: visit Instr 28: " << Broken << "\n";

    Assert(MD->getNumOperands() == 1, "fpmath takes one operand!", &I);

//    errs() << "Verifier: visit Instr 29: " << Broken << "\n";
...
//  errs() << "Verifier: visit Instr 30: " << Broken << "\n";
...
//  errs() << "Verifier: visit Instr 31: " << Broken << "\n";
...
//  errs() << "Verifier: visit Instr 32: " << Broken << "\n";

  if (MDNode *MD = I.getMetadata(LLVMContext::MD_dereferenceable))
    visitDereferenceableMetadata(I, MD);

//  errs() << "Verifier: visit Instr 33: " << Broken << "\n";

  if (MDNode *MD = I.getMetadata(LLVMContext::MD_dereferenceable_or_null))
    visitDereferenceableMetadata(I, MD);

//  errs() << "Verifier: visit Instr 34: " << Broken << "\n";

  if (MDNode *TBAA = I.getMetadata(LLVMContext::MD_tbaa))
    TBAAVerifyHelper.visitTBAAMetadata(I, TBAA);

//  errs() << "Verifier: visit Instr 35: " << Broken << "\n";

  if (MDNode *AlignMD = I.getMetadata(LLVMContext::MD_align)) {

//    errs() << "Verifier: visit Instr 36: " << Broken << "\n";
 
    Assert(I.getType()->isPointerTy(), "align applies only to pointer types",
           &I);

//    errs() << "Verifier: visit Instr 37: " << Broken << "\n";

    Assert(isa<LoadInst>(I), "align applies only to load instructions, "
           "use attributes for calls or invokes", &I);

//    errs() << "Verifier: visit Instr 38: " << Broken << "\n";

    Assert(AlignMD->getNumOperands() == 1, "align takes one operand!", &I);

//    errs() << "Verifier: visit Instr 39: " << Broken << "\n";
...
//  errs() << "Verifier: visit Instr 42: " << Broken << "\n";

  if (MDNode *N = I.getDebugLoc().getAsMDNode()) {
//    errs() << "Verifier: visit Instr 43: " << Broken << "\n";
    AssertDI(isa<DILocation>(N), "invalid !dbg metadata attachment", &I, N);
//    errs() << "Verifier: visit Instr 44: " << Broken << "\n";
    visitMDNode(*N);
  }

//  errs() << "Verifier: visit Instr 45: " << Broken << "\n";

  if (auto *DII = dyn_cast<DbgVariableIntrinsic>(&I))
    verifyFragmentExpression(*DII);

//  errs() << "Verifier: visit Instr 46: " << Broken << "\n";

  InstsInThisBlock.insert(&I);

//  errs() << "Verifier: visit Instr 47: " << Broken << "\n";
}
