...
const int CosineMetrics = 1;
const int ManhattonMetrics = 2;
      ...
      if (I.getParent()->getTerminator()) {
        ...
        if ((&I) == I.getParent()->getTerminator() && II != nullptr) {
          ...
          if (DestBB->getSinglePredecessor() == nullptr) {
            ...
            for (auto it = pred_begin(DestBB), et = pred_end(DestBB); it != et;
                 ++it) {
              BasicBlock *BB = *it;
              if (BB == SrcBB) {
  ...
  case Instruction::GetElementPtr: {
    ...
    */
    ...
    for (; CurIdx != Indices1.size(); ++CurIdx) {
      ...
      if (!IdenticalType) {
        const DataLayout *DT =
            &((Module *)GEP1->getParent()->getParent()->getParent())
                 ->getDataLayout();
...
#pragma omp parallel for
      ...
      for (unsigned j = 1; j < NumCols; j += blockIncr) {
        ...
        for (unsigned xi = i; xi < limitRows; xi++) {
          ...
          for (unsigned xj = j; xj < limitCols; xj++) {
            ...
            int similarity = Match[(xi - 1) * F2.size() + (xj - 1)]
                                 ? matchAward
                                 : mismatchPenalty;
  ...
  int calcScore(unsigned i, unsigned j) {
    // int similarity = match(F1[i - 1], F2[j - 1]) ? matchAward :
    // mismatchPenalty;
    int similarity =
        Match[(i - 1) * F2.size() + (j - 1)] ? matchAward : mismatchPenalty;
...
void optimizeAlignment(std::list<std::pair<Value *, Value *>> &AlignedInsts) {
  ...
  while (NextIt != AlignedInsts.end()) {
    if (It->first != nullptr && It->second != nullptr) {
      countAlignedSize++;
      if ((NextIt->first == nullptr || NextIt->second == nullptr) &&
          countAlignedSize == 1) {
        NewAlignedInsts.push_back(
            std::pair<Value *, Value *>(It->first, nullptr));
        NewAlignedInsts.push_back(
            std::pair<Value *, Value *>(nullptr, It->second));
        countAlignedSize = 0;
      } else {
      ...
      if (isa<BasicBlock>(Pair.first)) {
        ...
        if (MergedBB == nullptr) {
          ...
          /*
                    I1->dump();
                    errs() << "Tail: " <<
             GetValueName(dyn_cast<BasicBlock>(I1->getParent())) << " -> " <<
             GetValueName(TailBBs[ dyn_cast<BasicBlock>(I1->getParent()) ]) <<
             "\n"; I2->dump(); errs() << "Tail: " <<
             GetValueName(dyn_cast<BasicBlock>(I2->getParent())) << " -> " <<
             GetValueName(TailBBs[ dyn_cast<BasicBlock>(I2->getParent()) ]) <<
             "\n";
          */
          {
        ...
        /*
                for (Instruction &I : *MergedBB) {
                  if (isa<LandingPadInst>(&I) &&
           MergedBB->getFirstNonPHI()!=(&I)) { NewI->dump(); errs() << "Broken
           BB: 1\n"; MergedBB->dump();
                  }
                }
        */
      ...
      if (isa<Instruction>(Pair.first)) {
        ...
        /*
                for (Instruction &I : *NewI->getParent()) {
                  if (isa<LandingPadInst>(&I) &&
           NewI->getParent()->getFirstNonPHI()!=(&I)) { NewI->dump(); errs() <<
           "Broken BB: 3\n"; NewI->getParent()->dump();
                  }
                }
        */
...
void static UpdateCallGraph(Module &M, MergedFunction &Result,
  ...
  if (CanEraseF1 && (F1->getNumUses() == 0) &&
      (HasWholeProgram ? true : F1->hasLocalLinkage()) &&
  ...
  if (CanEraseF2 && (F2->getNumUses() == 0) &&
      (HasWholeProgram ? true : F2->hasLocalLinkage()) &&
...
double manhattonDistance(std::vector<double> &A, std::vector<double> &B) {
  double MD = 0.0;
  size_t len = A.size();
  for (size_t i = 0; i < len; i++) {
    MD += std::abs(A[i] - B[i]);
...
std::vector<double> fingerPrintToVector(Fingerprint *fp) {
  std::vector<double> FeatureVec;
  for (size_t i = 0; i < fp->MaxOpcode; i++) {
    FeatureVec.push_back(fp->OpcodeFreq[i]);
  }
  return FeatureVec;
}

std::vector<double>
getRefVec(std::map<Function *, Fingerprint *> &CachedFingerprints) {
  std::vector<double> RefVec(Fingerprint::MaxOpcode);
  for (auto &item : CachedFingerprints) {
    for (size_t i = 0; i < Fingerprint::MaxOpcode; i++) {
      RefVec[i] += item.second->OpcodeFreq[i];
    }
  }
  double Freq = CachedFingerprints.size();
  for (auto &num : RefVec) {
    num /= Freq;
  }
  return RefVec;
...
////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////
size_t FunctionMerging::mergeByPairwise(Module &M) {
  AlwaysPreserved.clear();
  AlwaysPreserved.insert("main");
  ...
  for (auto &F : M) {
    ...
    if (BlackList.count(std::string(F.getName())))
      continue;
  ...
  std::vector<double> RefVec = getRefVec(CachedFingerprints);
  ...
  return MergingDistance.size();
}

size_t FunctionMerging::mergeBySimilarity(Module &M, int Metrics) {
  ...
  // Start of cosine filter
  std::vector<double> RefVec = getRefVec(CachedFingerprints);
  std::vector<std::pair<Function *, double>> Scores;
  for (auto &item : CachedFingerprints) {
    auto FeatureVec = fingerPrintToVector(item.second);
    double Score;
    if (Metrics == CosineMetrics) {
      Score = cosineSimilarity(FeatureVec, RefVec);
    } else if (Metrics == ManhattonMetrics) {
      Score = manhattonDistance(FeatureVec, RefVec);
    }
    Scores.push_back({item.first, Score});
  }
  auto cmp = [](const std::pair<Function *, double> &a,
                const std::pair<Function *, double> &b) {
    return a.second < b.second;
  };
  std::sort(Scores.begin(), Scores.end(), cmp);
  std::vector<std::vector<Function *>> Groups;
  double Interval = 260;
  // double Interval = 0.05;
  auto Start = Scores.begin();
  while (Start != Scores.end()) {
    std::vector<Function *> G;
    G.push_back(Start->first);
    auto prev = Start;
    auto End = Start + 1;
    while (End != Scores.end() && End->second - prev->second < Interval) {
      G.push_back(End->first);
      prev = End;
      End++;
    }
    Groups.push_back(G);
  ...
  for (auto &Group : Groups) {
    /**
     * 1. Find all candidates and sort by similarity
     * 2. Linearize function and try to merge
     */
    std::list<Function *> WorkList, AvailableFuncs;
    for (Function *F : Group) {
      WorkList.push_back(F);
      AvailableFuncs.push_back(F);
    }

    while (!WorkList.empty()) {
      Function *F1 = WorkList.front();
      WorkList.pop_front();
      AvailableFuncs.erase(
          std::find(AvailableFuncs.begin(), AvailableFuncs.end(), F1));
      auto FeatureVecF1 = fingerPrintToVector(CachedFingerprints[F1]);
      std::vector<std::pair<Function *, double>> Candidates;
      for (Function *F : AvailableFuncs) {
        auto FeatureVecF = fingerPrintToVector(CachedFingerprints[F]);
        if (validMergeTypes(F1, F) && !filterMergePair(F1, F)) {
          Candidates.push_back(
              {F, cosineSimilarity(FeatureVecF1, FeatureVecF)});
        }
      }
      if (Candidates.empty()) {
        continue;
      ...
      Function *F2 = Candidates.front().first;
      if (Debug || Verbose) {
        errs() << "Attempting: " << GetValueName(F1) << ", " << GetValueName(F2)
               << "\n";
      }
      SmallVector<Value *, 32> F1Vec, F2Vec;
      Linearization(F1, F1Vec, LinearizationKind::LK_Canonical);
      Linearization(F2, F2Vec, LinearizationKind::LK_Canonical);
      MergedFunction Result = mergeBySequenceAlignment(F1, F2, F1Vec, F2Vec);
      if (Result.MergedFunc != nullptr && verifyFunction(*Result.MergedFunc)) {
      ...
      if (Result.MergedFunc) {
        errs() << "valid merge, check code size\n";
        DominatorTree MergedDT(*Result.MergedFunc);
        promoteMemoryToRegister(*Result.MergedFunc, MergedDT);
        double F1Size = FuncSizes[F1];
        double F2Size = FuncSizes[F2];
        double MergedSize = requiresOriginalInterfaces(Result) * 3 +
                            estimateFunctionSize(*Result.MergedFunc, &TTI);
        if (Debug || Verbose) {
          errs() << "Sizes: " << F1Size << " + " << F2Size
                 << " <= " << MergedSize << "?\n";
        }
        if (MergedSize >
            (F1Size + F2Size) * (100.0 + MergingOverheadThreshold) / 100.0) {
          Result.MergedFunc->eraseFromParent();
        } else { // we merge
          if (Debug || Verbose) {
            errs() << "Merged: " << GetValueName(F1) << ", " << GetValueName(F2)
          ...
          MergeCount++;
          AvailableFuncs.erase(
              std::find(AvailableFuncs.begin(), AvailableFuncs.end(), F2));
          WorkList.erase(std::find(WorkList.begin(), WorkList.end(), F2));
          UpdateCallGraph(M, Result, AlwaysPreserved);
          WorkList.push_back(Result.MergedFunc);
          AvailableFuncs.push_back(Result.MergedFunc);
          FuncSizes[Result.MergedFunc] =
              estimateFunctionSize(*Result.MergedFunc, &TTI);
          demoteRegToMem(*Result.MergedFunc);
          CachedFingerprints[Result.MergedFunc] =
...
bool FunctionMerging::runOnModule(Module &M) {
  ...
  size_t MergeCount = 0;
  // MergeCount += mergeBySimilarity(M, CosineMetrics);
  MergeCount += mergeBySimilarity(M, ManhattonMetrics);
  // MergeCount += mergeByPairwise(M);
  auto EndTime = std::chrono::high_resolution_clock::now();
  errs() << "Total number of merges: " << MergeCount << "\n";
  errs() << "Time used for module is "
